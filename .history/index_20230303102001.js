...
function checkWin(){
    for(let row = rows - 1; row > -1; row--){
        for(let column = 0; column < columns; column++){
            ...
            if(column + 3 < columns -1 && player == grid[row][column + 1] && player == grid[row][column+2] && player == grid[row][column+3]){
                return player;
            }
            if(row + 3 < rows && player == grid[row + 1][column] && player == grid[row + 2][column] && player == grid[row +3 ][column]){
                return player;
            }
            if(column + 3 < columns -1 && player == grid[row + 1][column + 1] && player == grid[row + 2][column+2] && player == grid[row +2][column+3]){
                return player
            }
